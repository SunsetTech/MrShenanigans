...
return function(SharedData)
	return function()
		...
		local wm_delete_window = ffi.new("uint64_t[1]", {X11.XInternAtom(Display, "WM_DELETE_WINDOW", false)});
		local XErrorHandler = function(Display, ErrorEvent)
			print("X11 error caught", Display, ErrorEvent)
		end
		while true do
